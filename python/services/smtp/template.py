...
class EmailElement(Element):
    loader = XMLFile(FilePath('/home/ubuntu/scewpt/build/xml/email_template.xml'))

    def __init__(self, helo, origin, user):
        print 'create element', helo, origin, user
        self.broadcast_dict = mail_keys.blank_msg()
    ...
    def raw_email(self, in_email):
        ...
        self.broadcast_dict['msg'] = decode_rfc2047_header(
            self.incoming_email.get("message-id"))

        ds = time.strftime(mail_keys.dt_format, time.localtime(self.broadcast_dict['date']))
        file_hash = ''.join(random.choice(string.ascii_lowercase) for x in range(6))
        file_format = mail_keys.version + '/%s_%s' % (ds, file_hash)
            ...
            if p.is_multipart():
                ...
                if content_type == "text/html":
                    ...
                    try:
                        ...
                        filename = re.findall(
                            "filename=(\S+)", p['content-disposition'])[0].replace('\"', '')
                        print 'filename:', filename
                        try:
                            filename = p.get_param(
                                'filename', None, 'content-disposition')
                        except Exception as e2:
                            print 'deep exception 1:', e2
                            filename = re.search(
                                mail_keys.email_filename, filename).group(2)
                    except Exception as e:
                ...
                def filename_link(f):
                    if len(f) > 0:
                        t = tags.a(f)
                        t.attributes[
                            'href'] = 'http://www.scewpt.com/' + self.broadcast_dict['file_dest'] + "_" + f
                        return t
                    else:
                        return f
